...
function validatePlace(raw, errors) {
  ...
  if (!name) {
    errors.push({ field: 'place.name', message: 'Name the place this happened.' });
  } else if (name.length > 160) {
    errors.push({ field: 'place.name', message: 'Please keep the place name under 160 characters.' });
  }

  if (!Number.isFinite(lat) || !Number.isFinite(lng)) {
    errors.push({ field: 'place.point', message: 'Drop a pin on the map to set the exact spot.' });
    return null;
  }
  if (!isInsideIran(lat, lng)) {
    errors.push({ field: 'place.point', message: 'That point is outside Iran. This map only carries narratives placed inside the country.' });
    return null;
  }
...
function validatePeriod(raw, errors) {
  ...
  if (!isRealDate(start) || !isRealDate(end)) {
    errors.push({ field: 'period', message: 'Give a real start and end date for the period.' });
    return null;
  }
  if (start > end) {
    errors.push({ field: 'period', message: 'The period ends before it starts.' });
    return null;
  }
  ...
  if (startYear < MIN_YEAR) {
    errors.push({ field: 'period', message: `This map covers ${MIN_YEAR} onwards.` });
    return null;
  }
  if (endYear > maxYear()) {
    errors.push({ field: 'period', message: 'The period cannot end in the future.' });
    return null;
  }
...
function validateContributor(raw, errors) {
  ...
  if (name.length > 80) {
    errors.push({ field: 'contributor.name', message: 'Please keep the name under 80 characters.' });
  }
  if (email && !EMAIL_PATTERN.test(email)) {
    errors.push({ field: 'contributor.email', message: 'That does not look like an email address.' });
  }
